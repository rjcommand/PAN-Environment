...
##
## Purpose of script:
## Preliminary analysis and plotting of Air Quality dataset worldwide 380 cities towards PAN-E
##
...
cities_remap = read.csv("cities_remap.csv")
print("renaming cities")
for(i in 1:length(cities_remap$old_cities)){
  print(i/length(cities_remap$old_cities))
  air_quality$City[air_quality$City == cities_remap$old_cities[i]] <- cities_remap$new_cities[i]
}
...
for(specie in c('aqi','co','no2','o3','pm1','pm10','pm25','so2')){
  ...
  if(length(aq_species_cities)> 30 & specie != 'aqi'){
    ...
    air_quality_specie_cities = air_quality_specie
  }
